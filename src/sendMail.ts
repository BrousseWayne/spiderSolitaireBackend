...
export async function sendPasswordRecoveryMail(
  sendTo: string,
  resetLink: string
) {
...
export default async function sendPasswordVerificationMail(
  sendTo: string,
  ...
  <html lang="en">
    <head>
      <meta charset="UTF-8" />
      <title>Mail address verification</title>
      <style>
        body {
          font-family: Arial, sans-serif;
          background-color: #f9fafb;
    ...
    <body>
      <div class="container">
        ...
        <div class="paragraph">
           Thank you for registering. Please verify your email address by clicking the button below. If you did not create an account, you can safely ignore this email.
        </div>
<a href="http://localhost:5173/verify-email?token=${verificationToken}" class="button" target="_blank" rel="noopener noreferrer">Verify Email</a>        <div class="paragraph" style="margin-top: 24px">
          Or copy and paste the following link into your browser:
        </div>
        <div class="code">http://localhost:5173/verify-email?token=${verificationToken}</div>
        <div class="footer">This link will expire in 20 minutes.</div>
      </div>
    </body>
  </html>
  ...
  const emailParams = new EmailParams()
    ...
    .setHtml(htmlContent)
    .setText(
      `To verify your email, visit the following link: ${verificationToken}`
    );
